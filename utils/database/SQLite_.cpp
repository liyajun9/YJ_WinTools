...
bool YSQLite::ExecuteSQLW(const std::wstring& sSQL, unsigned int* pAffectedRows/* = NULL*/)
{
	std::string sSQLmb = NS_Yutils::WCharToMB(sSQL);
	return ExecuteSQLA(sSQLmb, pAffectedRows);
}

bool YSQLite::ExecuteSQLA(const std::string& sSQL, unsigned int *pAffectedRows /*= NULL*/)
{
...
bool YSQLite::GetIsExistW(const std::wstring& sSQL)
{
	std::string sSQLmb = NS_Yutils::WCharToMB(sSQL);
	return GetIsExistA(sSQLmb);
}

bool YSQLite::GetIsExistA(const std::string& sSQL)
{
	std::string sSQLutf8 = NS_Yutils::MBToUtf8(sSQL);
...
int YSQLite::GetIntFieldW(const std::wstring& sSQL)
{
	std::string sSQLmb = NS_Yutils::WCharToMB(sSQL);
	return GetIntFieldA(sSQLmb);
}

int YSQLite::GetIntFieldA(const std::string& sSQL)
{
	std::string sSQLutf8 = NS_Yutils::MBToUtf8(sSQL);
...
std::wstring YSQLite::GetStringFieldW(const std::wstring& sSQL)
{
	std::string sSQLmb = NS_Yutils::WCharToMB(sSQL);
	std::string sRetmb = GetStringFieldA(sSQLmb);
	return NS_Yutils::MBToWChar(sRetmb);
}

std::string YSQLite::GetStringFieldA(const std::string& sSQL)
{
	std::string sSQLutf8 = NS_Yutils::MBToUtf8(sSQL);
...
bool YSQLite::ExecuteTransacW(const TransactSQLsW& vecSQL)
{

}

bool YSQLite::ExecuteTransacA(const TransactSQLsA& vecSQL)
{
	YCSLocker locker(m_cs);
	sqlite3_stmt *stmt = NULL;
	char *pszErrMsg = NULL;
	...
	try{
		...
		for(TransactSQLsA::const_iterator iter = vecSQL.begin(); iter != vecSQL.end(); ++iter){
			const std::string& sSQL = (std::string)(*iter);
			std::string sSQLutf8 = NS_Yutils::MBToUtf8(sSQL);
			if(nRes = sqlite3_exec(pDbconnection, sSQLutf8.c_str(), NULL, NULL, &pszErrMsg) != SQLITE_OK)
				throw YSQLiteException(pszErrMsg, sSQL.c_str(), "ExecuteTransac");
		}
...
bool YSQLite::GetIsTableExist(const std::string& sTable)
{
...
bool YSQLite::GetIsColumnExist(const std::string& sTable, const std::string& sCol)
{
	tostringstream ssSQL;
